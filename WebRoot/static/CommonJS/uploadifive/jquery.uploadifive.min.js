            ...
            return this.each(function() {
                    ...
                    function() {
                        ...
                        if ((d.queue.count + l) > f.queueSizeLimit && f.queueSizeLimit !== 0) {
                            if (b.inArray("onError", f.overrideEvents) < 0) {
                                //alert("98 The maximum number of queue items has been reached (" + f.queueSizeLimit + ").  Please select fewer files.");
                                //my
                                alert("最多只能上传 " + f.queueSizeLimit + " 个文件.");
                            }
                ...
                d.drop = function(m) {
                    ...
                    if ((d.queue.count + j) > f.queueSizeLimit && f.queueSizeLimit !== 0) {
                        if (b.inArray("onError", f.overrideEvents) < 0) {
                            //alert("140 The maximum number of queue items has been reached (" + f.queueSizeLimit + ").  Please select fewer files.");
                            //my 
                            alert("最多只能上传 " + f.queueSizeLimit + " 个文件");
                        }
                        if (typeof f.onError === "function") {
                ...
                if (f.itemTemplate == false) {
                    d.queueItem = b('<div class="uploadifive-queue-item" >                        <a class="close" href="#">×</a>                        <div><span class="filename"><span class="filesize"></span></span><span class="fileinfo"></span><input type="hidden" class="hidComplate"></span></div>                        <div class="progress">                            <div class="progress-bar"></div>                        </div>                    </div>');
                } else {
                ...
                d.addQueueItem = function(k) {
                    if (b.inArray("onAddQueueItem", f.overrideEvents) < 0) {
                        ...
                        function () {
                            var x_id = k.queueItem.find(".close")[0].id;
                            a.cancel.call(g, k);
                            deleteDocumentForUploadiFive(x_id);//my
                            //uploadifiveBorderChe();//my
                            return false;
                        });
                        var m = k.name;
                    ...
                    if (k.size > f.fileSizeLimit && f.fileSizeLimit != 0) {
                        ...
                        //my start
                        alert("文件大小不能超过" + (f.fileSizeLimit / 1024 / 1024) + "M");
                        a.cancel.call(g, k);
                ...
                d.uploadFile = function (k, l) {
                    //my start
                    if (typeof f.onCheckCount === "function") {
                        var countResult = f.onCheckCount.call();
                        if (!countResult)
                        {
                            //k.queueItem.find(".close").click();
                ...
                d.uploadComplete = function(l, j, k) {
                    if (b.inArray("onUploadComplete", f.overrideEvents) < 0) {
                        ...
                        var myFileObj = eval("(" + j.xhr.responseText + ")");
                        //my start
                        j.queueItem.find(".filename").attr("id", "newfilename_" + myFileObj.DocMuid);
                        j.queueItem.find(".close").attr("id", myFileObj.DocMuid);
                        j.queueItem.find(".hidComplate").attr("id", "hidNewFileName_" + myFileObj.DocMuid);
                        var myFileInfo = myFileObj.DocName + "~!~!" + myFileObj.DocMuid + "~!~!" + myFileObj.FilePath;
                        j.queueItem.find(".hidComplate").val(myFileInfo);//my 记录上传成功
                        //my end
